...
{
    public class PostGeneratePDF(PosteStorage posteStorage) : IPostGeneratePDF
    {
     

        public async Task<byte[]> GeneratePDF(int equipeId, DateTime date)
        {
            var (employePosts, equipeSalaires) = await posteStorage.SelectEquipeSalairesAndPostes(equipeId, date);
